...
onMounted(async () => {
  ...
  items.value = events.map(({ id, name, description, location, dances, clubId, startTime }) => ({
    id,
    content: `<h4>${name}</h4><p>${description}</p><span>${location.name} - ${new Date(startTime)}</span>`,
